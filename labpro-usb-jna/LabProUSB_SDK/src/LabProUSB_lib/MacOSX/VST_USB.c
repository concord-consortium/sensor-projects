...
bool	local_ConvertBytesToInt(unsigned char chLSB,
...
OSStatus local_ReadBytes(VST_USBBulkDevice * pUSBDevice, void * pBuffer, UInt32 * pnMaxWanted);
...
OSStatus VST_OpenUSBPortForIO(VST_USBBulkDevice * pUSBDevice)
{
	...
	if (pUSBDevice && pUSBDevice->ioUSBDeviceRef)
	{
		if (DIAGNOSTICS_ON)
			NSLog(CFSTR("*** VST_OpenUSBPortForIO *** 0x%x (0x%x), err=%0x0"), pUSBDevice, pUSBDevice->ioUSBDeviceRef, status);
	}
	else
		NSLog(CFSTR("*** VST_OpenUSBPortForIO *** 0x%x (NULL. Already been destroyed?), err=%0x0"), pUSBDevice, status);
	if (status != 0)
...
OSStatus VST_CloseUSBPort(VST_USBBulkDevice * pUSBDevice)
{
	...
	{
		if (DIAGNOSTICS_ON)
			NSLog(CFSTR("*** VST_CloseUSBPort *** 0x%x (0x%x)."), pUSBDevice, pUSBDevice->ioUSBDeviceRef);
	}
	else
		NSLog(CFSTR("*** VST_CloseUSBPort *** 0x%x (NULL. Already been destroyed?)."), pUSBDevice);
...
Boolean local_USBDeviceFromService(VST_USBBulkDevice * pUSBDevice, io_service_t ioSvc_usbDevice)
{
	...
	pUSBDevice->bPleaseExitListenerThread = false;
	pUSBDevice->bListenerThreadExited = false;
    pUSBDevice->nError = 0;
    pUSBDevice->ioUSBDeviceRef = ioSvc_usbDevice;
    pUSBDevice->pConfigurationDesc = 0;
    pUSBDevice->ioUSBInterfaceRef = 0;
    pUSBDevice->nControlPipeRef = pUSBDevice->nInPipeRef = pUSBDevice->nOutPipeRef = 0;
    pUSBDevice->nMaxInPacketSize = pUSBDevice->nMaxOutPacketSize = 0;
    pUSBDevice->bUsingSubPackets = FALSE;
	...
	if (ioSvc_interface != 0)
		pUSBDevice->ioUSBInterfaceRef = ioSvc_interface;	
	else
	{	// Report any failures at this point to the caller. (RM 2539) JK 20090209.
		NSLog(CFSTR("*** USBDeviceFromService: local_GetFirstInterfaceServiceFromDevice returned NULL for IOUSBDevice: 0x%x"), hIOUSBDevice);
		err = (*hIOUSBDevice)->USBDeviceClose(hIOUSBDevice);
		return false;
	}
    
    /* Get interface & pipe info... */
	if (!local_GetInterfaceInfoForUSBBulkDevice(pUSBDevice))
	{	// (RM 2539) JK 20090209.
		NSLog(CFSTR("*** USBDeviceFromService: local_GetInterfaceInfoForUSBBulkDevice failed. Dammit."));
		err = (*hIOUSBDevice)->USBDeviceClose(hIOUSBDevice);
		return false;
	}
        ...
        if (pUSBDevice->hInterfaceInterface == NULL)
        {
			...
			if (pUSBDevice->eBulkReadMode != kBulkReadDirect)
			{
				...
				err = (OSStatus) pthread_create(&pUSBDevice->listeningThread, 
											NULL, /* default attributes */
											(void* (*)(void*))local_input_listen_thread, /* function */
...
OSStatus local_CloseIO(VST_USBBulkDevice * pUSBDevice)
{
    ...
    if (pUSBDevice->listeningThread != NULL)
    {
		...
		pUSBDevice->bPleaseExitListenerThread = true;
        if (pUSBDevice->hInterfaceInterface != NULL)	
        {	//Force listener thread to exit blocking IO call.
            (*pUSBDevice->hInterfaceInterface)->USBInterfaceClose(pUSBDevice->hInterfaceInterface);
            (*pUSBDevice->hInterfaceInterface)->Release(pUSBDevice->hInterfaceInterface);
            pUSBDevice->hInterfaceInterface = NULL;
        }

		while (!(pUSBDevice->bListenerThreadExited))
		{
			nCt++;
			usleep(10000);
			if (nCt > 100)
			{
				printf("Listener thread did not exit. \n");
				break; 
			}
		}
                        ...
                        UInt32 * pnMaxWanted)
{
    ...
    err = (OSStatus) (*pUSBDevice->hInterfaceInterface)->ReadPipe(pUSBDevice->hInterfaceInterface, pUSBDevice->nInPipeRef, pBuffer, pnMaxWanted);                                            
    if (err == kIOUSBTransactionTimeout)
    {
        *pnMaxWanted = 0L;
        err = (*pUSBDevice->hInterfaceInterface)->ResetPipe(pUSBDevice->hInterfaceInterface, pUSBDevice->nInPipeRef);
    ...
    if (err == kIOUSBPipeStalled)
    {
        *pnMaxWanted = 0L;
        ...
        err = (*pUSBDevice->hInterfaceInterface)->ClearPipeStall(pUSBDevice->hInterfaceInterface, pUSBDevice->nInPipeRef);
...
//		NSLog(CFSTR("### --- dumped pending data."));
    ...
    while (bRun)
    {
        ...
        if ((pTheDevice->hInterfaceInterface != NULL) && (pTheDevice->nListenBufferMaxBytes > 0))
        {
			if (pTheDevice->eBulkReadMode == kBulkReadSmallPacketThreaded)
			{
				...
				bytes[0] = 0;
				bytes[64] = 0;
				err = local_ReadBytes(pTheDevice, bytes, &nNumBytes);
				pBytes = &bytes[0];
			}
			else if (pTheDevice->eBulkReadMode == kBulkReadLengthPacketThreaded)
			{
				...
				unsigned int headerSize = sizeof(GDeviceToHostUSBBulkPipePacketHeader);
				err = local_ReadBytes(pTheDevice, &header, &headerSize);
				if (err == 0 && headerSize == sizeof(GDeviceToHostUSBBulkPipePacketHeader))
				{
					...
					while (err == kIOReturnSuccess && nNumBytes < payloadLength)
					{
						...
						err = local_ReadBytes(pTheDevice, &pBytes[nNumBytes], &bytesRead);
						if (err == kIOReturnSuccess)
							nNumBytes += bytesRead;
					}
				}
				...
				{
					if (!(pTheDevice->bPleaseExitListenerThread))
            ...
            if ((err == 0) || (err == kIOUSBTransactionTimeout))
            {
            	...
            	if ((pBytes != NULL) && (nNumBytes > 0))
            	{
					...
					{	// Client has specified a bulk callback, so we'll send the data to the callback instead of buffering it.
						int nErr = pTheDevice->pBulkCallback(pBytes, nNumBytes, pTheDevice->pBulkCallbackContext);
						if (nErr != 0)
	...
	pTheDevice->bListenerThreadExited = true;
}
