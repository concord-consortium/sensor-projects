Please run copy_win32_wdapi_dll.exe once from this folder before the first time you run NGIO_VB_Form_Measure.exe.

This will put the correct 32 bit version of wdapi921.dll in the folder.

If you fail to do so, then you will get a fairly nasty error message complaining about failure to load NGIO_lib.dll. This is because NGIO_lib.dll is dependent on wdapi921.dll, and the loader could not find wdapi921.dll.
