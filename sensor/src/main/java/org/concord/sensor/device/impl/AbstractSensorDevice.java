...
public abstract class AbstractSensorDevice implements SensorDevice,
	...
	/**
	 * Override this if you device wants to use a different technique for
	 * automatically identify where the device is attached. One case is if the
	 * the software supports multiple types of "serialPorts" for example the
	 * LabPro can work with its one USB port, or with the rxtx serial port.
	 * 
	 * @return
	 */
	...
	/**
	 * By default this return the operating system serial port this is a bit
	 * confusing because it is really an object which can open the operating
	 * system serial ports not a particular serial port.
	 * 
	 * It should be overriden if different type of SensorSerialPort is needed by
	 * the device.
	 * 
	 * @return
	 */
	 ...
	 * returned from getSensorSerialPort. It can be overriden if the device
	 * needs to filter out or add some special ports.
	 * 
	 * @return
	 */
	...
	/**
	 * This is a utility method to deal with a common problem of matching up
	 * sensor requests with the sensor determined by probing the hardware return
	 * a score 0 to 100
	 * 
	 * @param request
	 * @return
	 */
	protected int compareSensorConfigAndRequest(SensorConfig config,
	 ...
	 * If it is not possible for some other reason like wrong sensors are
	 * attached it will return a config marked invalid.
	 * 
	 * @param request
	 * @param record
	 *            record is logged record being configured against 0xFF is the
	...
	protected void autoIdConfigureInternal(ExperimentConfigImpl expConfig,
		...
		for (int i = 0; i < sensorRequests.length; i++) {
			...
			for (int j = 0; j < sensorConfigs.length; j++) {
				scoreArray[j] = compareSensorConfigAndRequest(sensorConfigs[j],
						sensorRequests[i]);
			}
		...
		for (int i = 0; i < sensorRequests.length; i++) {
			...
			for (int j = 0; j < scoreArray.length; j++) {
				// once a sensor is found then in the future matches should
				// ignore it.
	...
	protected ExperimentConfig autoIdConfigure(ExperimentRequest request) {
		...
		ExperimentConfigImpl deviceConfig = (ExperimentConfigImpl) getCurrentConfig();
		...
		// more generally.
		// 7/12/2011 - should add flag for devices to enable or disable this option
		SensorRequest[] sensorRequests = request.getSensorRequests();
		if (supportsRawSensorValues() && sensorRequests != null && sensorRequests.length > 0
				&& (isRawType(sensorRequests[0].getType()))) {
			SensorConfig rawSensorConfig = createSensorConfig(sensorRequests[0]
					.getType(), 0);
		...
		if (deviceConfig.getSensorConfigs() == null) {
			...
			// are requested.
			// If the device doesn't support non auto id sensors then there is
			// no point
	...
	protected void nonAutoIdConfigureInteral(ExperimentConfigImpl deviceConfig,
			ExperimentRequest request) {
		SensorRequest[] sensorRequests = request.getSensorRequests();
		...
		for (int i = 0; i < sensorRequests.length; i++) {
			...
			sensorConfigs[i] = createSensorConfig(sensorRequests[i].getType(),
					sensorRequests[i].getPort());
		}
		deviceConfig.setSensorConfigs(sensorConfigs);
		deviceConfig.setValid(true);
	 ...
	 * can only handle its internal SensorConfig implementations this should be
	 * overriden to create on of those.
	 * 
	 * @param type
	 * @return
	 */
	...
	/**
	 * Subclasses should override this method if they support sensors that don't
	 * provide autoId. If this returns true, then the autoIdConfigure method
	 * check if the getCurrentConfig returns a device config that returns null
	 * on getSensorConfigs(). Then it will assume the device is has the
	 * requested sensors attached. It will call createSensorConfig with the type
	 * and port of the sensor requested.
	 * 
	 * TODO Currently this won't handled mixed sensors where some are autoid and
	 * some are not.
	 * 
	 * @return
	 */
	...
	/**
	 * Subclasses should override this method if they support sensors that are 
	 * uncalibrated. This is useful for making DIY sensors attached to a commercial interface.
	 * it also can be useful for debugging calibrated data.   
	 * 
	 * Currently if this is supported by the device the autoIdConfigureInternal method will call
	 * createSensorConfig to make a fake raw voltage sensor attached to the device, that is a HACK. 
	 * 
	 * @return
	 */
